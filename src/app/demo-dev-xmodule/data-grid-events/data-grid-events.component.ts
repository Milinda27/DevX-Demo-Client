...
@Component({
  selector: 'app-data-grid-events',
  templateUrl: './data-grid-events.component.html',
  styleUrls: ['./data-grid-events.component.scss']
})
export class DataGridEventsComponent implements OnInit {
  ...
  onCellChange($event: unknown):void {
      console.log($event);
  }
  
  onCellClick($event:{rowIndex:number,columnIndex:number,displayValue:string}):void{    
    alert(`selected row: ${$event.rowIndex} selected column: ${$event.columnIndex} displayValue : ${$event.displayValue}`)
  }

  calculateFreight(rowData:{Freight:number}):string{
    return `${rowData.Freight/10}%`
  }
  ...
  calculateFilterExpression(filterValue:any, selectedFilterOperation:any){
    // Override implementation for the "between" filter operation
      ...
      // return [['ShipCountry',selectedFilterOperation,filterValue]];
      return [['CustomerID',selectedFilterOperation,filterValue] && ['ShipCountry',selectedFilterOperation,filterValue]];
